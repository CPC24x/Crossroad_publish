...
public class ManufacturerExtendedService : ManufacturerService ,IManufacturerExtendedService
{
    ...
    public async Task<Core.Domain.Catalog.Manufacturer> GetManufacturerByNameAsync(string name) => await _manufacturerRepository.Table.FirstOrDefaultAsync(m => m.Name == name);
}